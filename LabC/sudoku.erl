...
%% refine function using the sequential refine_rows function
refine(M) ->
    NewM =
	refine_rows(
	  transpose(
...
%% refine function using the parallel refine_rows function
par_refine(M) ->
    NewM =
	par_refine_rows(
	  transpose(
...
%% Sequential version of refine_rows
refine_rows(M) ->
...
%% Parallel version of refine_rows
par_refine_rows(M) ->
		Parent = self(),
		List = [{make_ref(),N} || N <- M, N /= []],
		[spawn_link(fun () -> 
                    Parent ! {Id,par_refine_row(N)} 
                end) || {Id, N} <- List, N /= []],
		[receive {Ref, {Val,Msg}}
		-> case Val==ok of
			  true  -> Msg;
...
%% Parallel version of guesses, not currently used in code since it makes 
%% execution slower.
par_guesses(M) ->
    Parent = self(),
    {I,J,Guesses} = guess(M),
    List = [{make_ref(),G} || G <- Guesses],
    [spawn_link(fun() ->
            Parent ! {Id, catch refine(update_element(M,I,J,G))}
                end) 
      || {Id, G} <- List],
    Ms = [receive {Ref, Ans} -> Ans end || {Ref, _} <- List],
    SortedGuesses =
      lists:sort(
        [{hard(NewM),NewM}
         || NewM <- Ms,
...
%% Original benchmarks, runs the problems sequentially
benchmarks() ->
  {ok,Puzzles} = file:consult("problems.txt"),
  timer:tc(?MODULE,benchmarks,[Puzzles]).

%% Parallel benchmarks, runs every problem in parallel (Task 1)
pbenchmarks(Puzzles) ->
    Parent = self(),
    Ref = make_ref(),
    [spawn_link(fun() -> 
     ...
     || {_,M} <- Puzzles],
    [receive {Ref, Ans} -> {Name, Ans} end || {Name,_} <- Puzzles].
